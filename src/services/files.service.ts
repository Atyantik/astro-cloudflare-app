import { filesTable, type InsertFileType } from "@db/schema/files";
...
export const createFile = async (
  file: Omit<InsertFileType, "id">,
  db: DrizzleD1Database,
) => {
  try {
    const response = await db.insert(filesTable).values({
      ...file,
      id: uuidv7(),
    });
    if (response.success) {
